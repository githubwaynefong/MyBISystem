...
public class LogParser {
	...
	//Apache服务器日志信息的正则表达式
	private static final String APACHE_LOG_REGEX = "^([0-9.]+)\\s"
		    +"([\\w.-]+)\\s"
		    +"([\\w.-]+)\\s"
		    +"\\[([^\\[\\]]+)\\]\\s"
		    +"\"((?:[^\"]|\\\")+)\"\\s"
	...
	public static final String CANNOT_GET = "can_not_get";
...
//			}
			...
			ipAddress = ipStr;
			//解析IP地址
			IpParser4LocalCache ipParser = new IpParser4LocalCache();
			try {
				//根据IP地址得出所在区域
				String[] parseStrs = ipParser.parse(ipStr).split(" ");
				areaAddress = parseStrs[0];
				localAddress = parseStrs[1];
			} catch (Exception e) {
				e.printStackTrace();
			} 
